...
class EmbeddingClient:
    ...
    def __init__(self, embedding_url: str = None):
        ...
        if not self.url:
            raise ValueError("Embedding URL must be provided either as an argument or via environment variable 'EMBEDDING_URL'.")
    ...
    def get_embeddings(self, text_list: List[str], model: str = "m3e", version: str = "m3e", unique_id: str = "test") -> dict:
        """
    ...
    def print_embeddings(self, text_list: List[str]) -> None:
        ...
        try:
            ...
            for text, embedding in zip(text_list, embeddings.get("embeddings", [])):
                print(f"Text: {text}")
